...
export async function addLightmapTexture(doc, pngBytes, { onProgress, mode = 'uastc' } = {}) {
  doc.createExtension(KHRTextureBasisu).setRequired(true);
  onProgress?.(`encoding lightmap (${mode === 'etc1s' ? 'ETC1S' : 'UASTC quality 4'}, sRGB, original dimensions, no mips)`);
  ...
  const out = await encodeKtx2(pngBytes, { mode, srgb: true, mipmaps: false, maxSize: Infinity, uastcLevel: 4, zstd: 18 });
