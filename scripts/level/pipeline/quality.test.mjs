...
test('presets: low has no lightmap, medium is the cheap Cycles budget, high defers to the level', () => {
  assert.equal(QUALITY_PRESETS.low.baker, 'none');
  assert.deepEqual(QUALITY_PRESETS.low.textures, { maxSize: 1024, maxFace: 1024 });
  assert.deepEqual(QUALITY_PRESETS.medium, { baker: 'blender', lightmap: { size: 2048, samples: 16, noiseThreshold: null } });
...
test('the texture budget caps a tier below the level and never raises it', () => {
  ...
  assert.deepEqual(textureBudgetFor({ maxSize: 2048 }, QUALITY_PRESETS.low), { maxSize: 1024, maxFace: 1024 });
  assert.deepEqual(textureBudgetFor({ maxSize: 512 }, QUALITY_PRESETS.low), { maxSize: 512, maxFace: 1024 });
