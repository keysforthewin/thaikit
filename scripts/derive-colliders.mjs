...
import { colliderPartCeiling } from './lib/budget.mjs';
 ...
 */
function loadParts(file, maxTriangles) {
  const mod = { exports: {} };
 ...
 */
function filterDecoration(parts, bounds, opts) {
 ...
 */
function voxelise(parts, bounds, voxel, maxCells) {
  let v = voxel;
  let nx;
  let ny;
  let nz;
 ...
 */
function largestRect(mask, nx, nz) {
...
function coverWithRects(mask, nx, nz, maxRects, minCells) {
...
function boxFromRect(grid, rect, y0, y1, name) {
 ...
 */
function prune(parts, maxParts, notes) {
 ...
 */
function selfCheck(grid, parts, geometryParts) {
 ...
 */
function ledgeInventory(grid, parts) {
...
function deriveOne(asset, opts) {
  ...
  const maxParts = opts.maxParts ?? colliderPartCeiling(asset);
  const notes = [];
...
function suggestMass(asset, parts) {
 ...
 */
function measureOne(asset, doc, opts) {
  const started = Date.now();
  const dir = opts.from === 'scratch' ? workDir(asset.id) : assetDir(asset.id);
  const bundle = path.join(dir, 'model.bundle.js');
  if (!existsSync(bundle)) throw new Error(`no bundle at ${toRepoRelative(bundle)}`);
