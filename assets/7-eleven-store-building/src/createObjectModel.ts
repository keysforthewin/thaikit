...
export function create7ElevenStoreBuildingModel(options: ProceduralModelOptions = {}): THREE.Group {
  ...
  socket_building_shell_socket_0_0.position.set(0.0, 3.54, 3.5);
  socket_building_shell_socket_0_0.rotation.set(0, 0, 0);
  socket_building_shell_socket_0_0.userData.socket = {"name": "sign-mount", "localPosition": [0, 3.54, 3.5], "axis": [0, 0, 1], "confidence": 0.85, "notes": "Where the brand fascia attaches. This is a REAL mechanism with a real consumer: fourteen buildings in this kit share the identical 8.0 x 4.6 x 7.0 m module and differ only in the sign hung here, so the mount is a contract the kit already relies on."};
  ...
  const mesh_facade_wall_1Geometry = endpoint_facade_wall_1
    ...
    : buildExtrudeGeometry({"points": [[-4.0, 0.0], [4.0, 0.0], [4.0, 4.25], [-4.0, 4.25]], "holes": [[[-3.0, 0.15], [3.0, 0.15], [3.0, 2.78], [-3.0, 2.78]]], "depth": 0.2});
  if (!endpoint_facade_wall_1) {
  ...
  const mesh_fascia_tray_8Geometry = endpoint_fascia_tray_8
    ...
    : new THREE.BoxGeometry(1, 1, 1, 4, 4, 4);
  if (!endpoint_fascia_tray_8) {
    mesh_fascia_tray_8Geometry.scale(6.6, 0.92, 0.18);
  }
  ...
  const mesh_fascia_graphic_face_9Geometry = endpoint_fascia_graphic_face_9
    ...
    : new THREE.BoxGeometry(1, 1, 1, 4, 4, 4);
  if (!endpoint_fascia_graphic_face_9) {
    mesh_fascia_graphic_face_9Geometry.scale(6.48, 0.8, 0.01);
  }
...
function applyGlazing(root: THREE.Group): void {
  ...
  for (const id of ['shopfront-glazing', 'side-window']) {
    ...
    m.transparent = true;
    m.opacity = 0.92;
    m.color = new THREE.Color('#B3BFC3');   // lifted from #8E9EA6: the plate's pane is a bright, near-white reflection, and the slate tone read as a dull blind panel. Luma ~185, far above the backdrop's 58.
    m.roughness = 0.08;
    m.metalness = 0.10;
    m.envMapIntensity = 1;
    m.depthWrite = true;
