...
function applyCanvasGraphics(root: THREE.Group): void {
  ...
  if (sign) {
    const { c, g } = makeGraphicCanvas(1024, 152);          // 5.40 x 0.80 m, ~6.75:1
    g.fillStyle = '#8FC72C';
    g.fillRect(0, 0, c.width, c.height);
    // CENTRE the whole mark. A first draft hard-coded the swoosh and wordmark at fixed x and
    // the group came out right of centre on the tray; the mark is measured and placed instead.
    g.fillStyle = '#FFFFFF';
    ...
    m.map = asTexture(c);
    // A `map` MULTIPLIES `color`. Leaving the authored #8FC72C on the material while the canvas
    // also paints #8FC72C squares the green and renders a vivid lightbox as dark olive -- and it
