...
test('unbaked cells are explicit and versioned; old manifests remain baked by default',()=>{
 const cell={key:'0_0',ix:0,iz:0,bounds:base.bounds,drawCalls:[1,1,1],triangles:[2,2,2]};
 assert.equal(ManifestExtras.parse({...base,cells:{size:24,list:[cell]}}).cells.list[0].bakeLighting,true);
 const next={...base,schemaVersion:3,cells:{size:24,list:[{...cell,node:'unbaked_0_0',bakeLighting:false}]}};
 assert.equal(ManifestExtras.parse(next).cells.list[0].bakeLighting,false);
 assert.throws(()=>ManifestExtras.parse({...next,schemaVersion:2}));
});
