/**
 * Motorcycle Lane Sign -- Vibe3D entry point.
 *
...
import {
  ...
  createMotorcycleLaneSignLookDevLights,
  frameMotorcycleLaneSignCamera,
...
export function createPreview(options: PreviewOptions = {}): ModelPreview {
  ...
  const lights = createMotorcycleLaneSignLookDevLights('neutral');
  ...
  let elapsed = time;
  const aim = () => {
    frameMotorcycleLaneSignCamera(camera, root, {
      margin,
      elevationDeg,
