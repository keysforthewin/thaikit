/**
 * Cafe Amazon Store Building -- Vibe3D entry point.
 *
...
import {
  ...
  createCafeAmazonStoreBuildingLookDevLights,
  frameCafeAmazonStoreBuildingCamera,
...
export function createPreview(options: PreviewOptions = {}): ModelPreview {
  ...
  const lights = createCafeAmazonStoreBuildingLookDevLights('neutral');
  ...
  let elapsed = time;
  const aim = () => {
    frameCafeAmazonStoreBuildingCamera(camera, root, {
      margin,
      elevationDeg,
