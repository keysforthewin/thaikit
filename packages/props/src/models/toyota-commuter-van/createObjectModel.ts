...
/** Standard UV-mapped PBR finish. Surface detail is stored entirely in standard glTF texture channels.
 * Six padded planar charts share the van's local coordinate frame; body stripes
 * remain vertex colours. The same maps render in Three.js and are embedded in GLB. */
function vanPortableFinish(meshes: THREE.Mesh[], material: THREE.MeshStandardMaterial, kind: 'paint' | 'steel') {
 const bounds=new THREE.Box3();
 for(const mesh of meshes){mesh.geometry.computeBoundingBox();bounds.union(mesh.geometry.boundingBox!);}
 const lo=bounds.min.toArray(),hi=bounds.max.toArray();
 const tile=256,pad=2,width=tile*4,height=tile*2;
 const chartAxes=[[2,1,0],[2,1,0],[0,2,1],[0,2,1],[0,1,2],[0,1,2]];
 for(const mesh of meshes){
  // Unique triangle vertices keep adjacent projection charts from sharing UVs.
  const old=mesh.geometry,g=old.index?old.toNonIndexed():old;
  const p=g.getAttribute('position'),n=g.getAttribute('normal'),uv=new Float32Array(p.count*2);
  for(let i=0;i<p.count;i+=3){
   const normal=[0,1,2].map(a=>[0,1,2].reduce((s,j)=>s+n.getComponent(i+j,a),0));
   const axis=normal.map(Math.abs).indexOf(Math.max(...normal.map(Math.abs)));
   const ch=axis===0?(normal[0]>0?0:1):axis===1?(normal[1]>0?2:3):(normal[2]>0?4:5);
   const [u,v]=chartAxes[ch];
   for(let j=0;j<3;j++){
    const su=(p.getComponent(i+j,u)-lo[u])/Math.max(hi[u]-lo[u],1e-6),sv=(p.getComponent(i+j,v)-lo[v])/Math.max(hi[v]-lo[v],1e-6);
    uv[(i+j)*2]=((ch%3)*tile+pad+su*(tile-2*pad))/width;
    uv[(i+j)*2+1]=1-(Math.floor(ch/3)*tile+pad+(1-sv)*(tile-2*pad))/height;
   }
  }
  g.setAttribute('uv',new THREE.BufferAttribute(uv,2));mesh.geometry=g;if(g!==old)old.dispose();
 }
 material.name=`commuter-${kind}`;
 if(typeof document==='undefined')return;
 const canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;
 const roughCanvas=document.createElement('canvas');roughCanvas.width=width;roughCanvas.height=height;
 const ctx=canvas.getContext('2d')!,roughCtx=roughCanvas.getContext('2d')!;
 const pixels=ctx.createImageData(width,height),rp=roughCtx.createImageData(width,height);
 const fract=(v:number)=>v-Math.floor(v),mix=(a:number,b:number,t:number)=>a+(b-a)*t;
 const smooth=(a:number,b:number,v:number)=>{const t=Math.max(0,Math.min(1,(v-a)/(b-a)));return t*t*(3-2*t);};
 const hash=(x:number,y:number,z:number)=>fract(Math.sin(x*12.9898+y*78.233+z*37.719)*43758.5453);
 const noise=(x:number,y:number,z:number)=>{
  const ix=Math.floor(x),iy=Math.floor(y),iz=Math.floor(z),fx=smooth(0,1,fract(x)),fy=smooth(0,1,fract(y)),fz=smooth(0,1,fract(z));
 ...
 const fbm=(x:number,y:number,z:number)=>.57*noise(x,y,z)+.28*noise(x*2.13,y*2.13,z*2.13)+.15*noise(x*4.31,y*4.31,z*4.31);
 const srgb=(v:number)=>Math.round(255*(v<=.0031308?12.92*v:1.055*Math.pow(v,1/2.4)-.055));
 for(let y=0;y<height;y++)for(let x=0;x<width;x++){
  if(x>=tile*3)continue; // Power-of-two atlas padding for Unreal texture import.
  const ch=Math.floor(y/tile)*3+Math.floor(x/tile),[a,b,ax]=chartAxes[ch];
  const p=[0,0,0];p[a]=mix(lo[a],hi[a],Math.max(0,Math.min(1,(x%tile-pad)/(tile-2*pad))));p[b]=mix(lo[b],hi[b],1-Math.max(0,Math.min(1,(y%tile-pad)/(tile-2*pad))));p[ax]=ch%2===0?hi[ax]:lo[ax];
  let col=[1,1,1],rough=material.roughness;
  if(kind==='paint'){
   const cloud=fbm(p[0]*19,p[1]*14,p[2]*16),streak=fbm(p[0]*48,p[1]*2,p[2]*43);
   const grime=(1-smooth(.31,.78,p[1]))*(.12+.38*streak),chalk=mix(.90,.78,smooth(1.8,2,p[1]))+.12*smooth(.25,.72,cloud);
   col=[.79,.73,.61].map(t=>chalk*(1-grime)*mix(1,t,grime*.35));
   rough=Math.max(.4,Math.min(.9,.62+.06*Math.sin(p[2]*27)*Math.sin(p[1]*39)+.10*(1-smooth(.32,.65,p[1]))));
  }else{
   const axle=p[2]>0?1.64:-1.62,q=[p[2]-axle,p[1]-.34],r=Math.hypot(...q);
   if(Math.abs(p[0])>.87&&r<.23){const k=.86+.1*noise(...p.map(v=>v*270) as [number,number,number])+.04*noise(...p.map(v=>v*48) as [number,number,number]);col=[.55,.47,.35].map(v=>k*mix(1,v,(1-smooth(.075,.105,r))*.55));}
  }
  const i=(y*width+x)*4;for(let j=0;j<3;j++){pixels.data[i+j]=srgb(col[j]);rp.data[i+j]=Math.round(rough*255);}pixels.data[i+3]=rp.data[i+3]=255;
 }
 ctx.putImageData(pixels,0,0);roughCtx.putImageData(rp,0,0);
 const map=new THREE.CanvasTexture(canvas);map.colorSpace=THREE.SRGBColorSpace;map.anisotropy=4;map.name=`commuter-${kind}-albedo`;
 const roughMap=new THREE.CanvasTexture(roughCanvas);roughMap.name=`commuter-${kind}-roughness`;
 material.map=map;material.roughnessMap=roughMap;material.roughness=1;material.needsUpdate=true;
}
...
export function createObjectModel(spec:any=VAN_SPEC, options:ProceduralModelOptions={}):THREE.Group {
 ...
 vanPortableFinish(Object.values(present).filter(m=>m.material===paint),paint,'paint');
 vanPortableFinish(Object.values(present).filter(m=>m.material===present.hardware.material),present.hardware.material as THREE.MeshStandardMaterial,'steel');
 root.name='Toyota Commuter Van';
