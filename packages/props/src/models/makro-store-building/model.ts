/**
 * Makro Store Building -- Vibe3D entry point.
 *
...
import {
  ...
  createMakroStoreBuildingLookDevLights,
  frameMakroStoreBuildingCamera,
...
export function createPreview(options: PreviewOptions = {}): ModelPreview {
  ...
  const lights = createMakroStoreBuildingLookDevLights('neutral');
  ...
  let elapsed = time;
  const aim = () => {
    frameMakroStoreBuildingCamera(camera, root, {
      margin,
      elevationDeg,
